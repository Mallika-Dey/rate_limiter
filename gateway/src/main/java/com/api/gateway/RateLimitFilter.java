...
public class RateLimitFilter extends AbstractGatewayFilterFactory<RateLimitFilter.Config> {
    ...
    private BucketConfiguration createBucketConfig(Config config) {
        ...
        Bandwidth perSecondLimit = Bandwidth.builder()
                .capacity(config.getRefillRatePerSecond())
                .refillGreedy(config.getRefillRatePerSecond(), Duration.ofSeconds(1))
                .build();
...
//        );
    ...
    public static class Config {
        ...
        private int refillRatePerSecond; //keep burstCapacity and refill rate same
